...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async changePassowrd(@ConnectedSocket() socket: Socket, @MessageBody() body: ChangePswDTO) {
		...
		this.server.to(socket.id).emit("success", "Password was succesfully changed !")
	}
	...
	async inviteToPlay(@ConnectedSocket() socket: Socket, @MessageBody() details: inviteToPlayDetails) {
		...
		if (targetedSockets) {
			this.server.to(targetedSockets).emit('notification', "You have been ivited to a game (" + json.mode + ") by " + json.sender_username + " !");
	...
	async muteUser(@ConnectedSocket() socket: Socket, @MessageBody() details: muteUserDetails) {
		...
		if (decoded.ft_id === details.receiverId)
			return (this.sendErrorMessage(socket.id, "receiveError", "Error: You can not mute yourself."))
		...
		if (await this.chatService.isMuted(details.receiverId, details.chanId) === "error")
			return (this.sendErrorMessage(socket.id, "receiveError", "Error: Wrong data types."))
		if (await this.chatService.isMuted(details.receiverId, details.chanId) === true)
			return (this.sendErrorMessage(socket.id, "receiveError", "Error: Targeted user is already muted in this channel."))
		if (details.time < 1)
		...
		this.server.to(socket.id).emit("success", username + " was successfully muted for " + details.time + " minute(s) !")
	}
	...
	async banUser(@ConnectedSocket() socket: Socket, @MessageBody() details: banUserDetails) {
		...
		this.server.to(socket.id).emit("success", username + " was successfully banned !")
	}
	...
	async unbanUser(@ConnectedSocket() socket: Socket, @MessageBody() details: banUserDetails) {
		...
		this.server.to(socket.id).emit("success", username + " was successfully unbanned !")
	}
	...
	async setAsAdmin(@ConnectedSocket() socket: Socket, @MessageBody() details: banUserDetails) {
		...
		if (newAdminSockets) {
			...
			this.server.to(newAdminSockets).emit('notification',  "You are now admin in this channel : " + chanName);
		}
		this.server.to(socket.id).emit("success", username + " is now an admin in this channel !")
	}
	...
	async sendFriendRequest(@ConnectedSocket() socket: Socket, @MessageBody() details: AddFriendDTO) {
		...
		if (isBlocked == false) {
			...
			if (targetedSockets)
				...
				this.server.to(targetedSockets).emit('notification', username + " wants to be your friend !")
		}
	}
	...
	async getFriendRequestResponse(@ConnectedSocket() socket: Socket, @MessageBody() details: friendRequestAnswer) {
		...
		if (details.answer === true) {
			...
			if (targetedSockets) {
				...
				this.server.to(targetedSockets).emit('notification', username + " accepted your friend request !")
			}	
	...
	async inviteToChannel(@ConnectedSocket() socket: Socket, @MessageBody() details: channelInvitationDetails) {
		...
		if (targetedSockets) {
			this.server.to(targetedSockets).emit('notification', username + " has invited you to a private channel : " + chanName + " !")
	...
	async handleConnection(client: Socket, ...args: any[]) {
		...
		if (client.handshake.query.jwt) {
			...
			if (decoded === "error")
				return (this.sendErrorMessage(client.id, "receiveError", "Error: Wrong data types."))
			if (!decoded)
