...
export class AuthService {
	...
	async createUser(details: UserDetails) {
		...
		while (await this.usernameExists(username_temp) === true)
	...
	async generate2fa(body: RecupDTO) {
		...
		if (userz.is2fa === true) {
			...
			await this.userRepo
    			...
    			.execute()
			return {status: "OK", message: "2fa was send"}
		}
		return {status: "KO", message: "No 2fa"}
	}
	...
	async changeTfa( tfa: TfaDTO) {
		...
		if (user) {
			...
			await this.userRepo
				...
				.where("ft_id= :id", { id: usernametoken.ft_id })
				.execute()
			return {status: "OK", message: "2fa was changed", is2fa: tfa.is_2fa}
			}
		}
		return {status: "KO", message: "You don't have email backup"}
	}
	...
	async verifyCode(body: VerifyCodeDTO) {
		...
		return {status: "KO", message: "Bad verify code"}
	}
}
